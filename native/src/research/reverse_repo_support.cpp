...
std::optional<double> number_value(const Json& value, std::string_view key) {
    ...
    try { std::size_t used = 0; const auto parsed = std::stod(text, &used);
...
std::string iso_date(const std::string& value) {
    return digits(value, 8) ? value.substr(0, 4) + "-" + value.substr(4, 2) +
        "-" + value.substr(6, 2) : value;
}
int market_id(std::string value) {
    ...
    throw Error("market must be all, sz, sh, bj, 0, 1, or 2");
}
...
Json security_document(int id, const std::string& code,
    ...
    Json result = Json::object(); result["market"] = market_name(id);
    result["market_id"] = id; result["code"] = code;
    result["security_id"] = market_prefix(id) + code;
    result["name"] = found == securities.end() ? "" : found->second.name;
    result["name_resolved"] = found != securities.end(); result["category"] = "repo";
    return result;
}
