...
Json count_json(const Json& value, std::string_view key) {
    const auto parsed = number_value(value, key);
    return parsed ? Json(*parsed) : Json(0);
}
...
std::string normalized_status(const std::string& raw) {
    if (raw == "入围") return "entered";
    if (raw == "跌出") return "exited";
    if (raw == "不变") return "continuing";
    return raw.empty() ? "unknown" : "upstream-other";
}

std::optional<double> json_number(const Json& row, std::string_view key) {
    const auto* value = value_ptr(row, key);
