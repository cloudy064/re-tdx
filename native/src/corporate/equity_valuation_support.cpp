...
std::string compact_date(std::string value, std::string_view name) {
    ...
    if (value.size() != 8 || !ascii_digits(value))
...
std::string years_before(const std::string& date, int years) {
    ...
    std::ostringstream output;
    output << std::setw(4) << std::setfill('0') << year << date.substr(4);
    auto result = output.str();
    if (result.substr(4) == "0229") result.replace(6, 2, "28");
    return result;
}
...
double decimal_value(const std::string& raw, std::string_view name,
    ...
    try {
        ...
        throw Error(std::string(name) + " must be a number in " +
...
int pe_type_value(std::string raw) {
    ...
    if (raw == "ttm" || raw == "1") return 1;
    if (raw == "annual" || raw == "report" || raw == "0") return 0;
    throw Error("pe_type must be ttm/1 or annual/0");
}

std::string pe_type_name(int value) { return value == 1 ? "ttm" : "annual"; }

int security_market(std::string raw) {
    raw = lower_ascii(trim(std::move(raw)));
    if (raw == "sz" || raw == "0") return 0;
    if (raw == "sh" || raw == "1") return 1;
    if (raw == "bj" || raw == "2") return 2;
