...
int command_market_hk_finance(const std::vector<std::string>& raw_args) {
    Args args(raw_args);
    if (args.take_flag("--help") || args.take_flag("-h")) {
        std::cout <<
            "Usage: tdx-tool market hk-finance [options]\n\n"
            "Decode the client's encrypted current HK finance cache.\n\n"
            "Options:\n"
            "  --code HKCODE          Exact five-digit HK security code\n"
            "  --query TEXT           Match any native source field\n"
            "  --classification CODE  Exact native classification code\n"
            "  --from DATE            Inclusive report-date lower bound\n"
            "  --to DATE              Inclusive report-date upper bound\n"
            "  --sort NAME            code|report_date|listing_date|classification\n"
            "  --order NAME           asc|desc (default asc)\n"
            "  --offset N             Result offset, 0..1000000\n"
            "  --limit N              1..2000 (default 200)\n"
            "  --root PATH            TDX installation root\n"
            "  --output PATH          Default output/tdx-market-hk-finance-native.json\n"
    ...
    query.classification = args.take_option("--classification");
    query.report_from = args.take_option("--from");
    query.report_to = args.take_option("--to");
    query.sort = args.take_option("--sort", "code");
