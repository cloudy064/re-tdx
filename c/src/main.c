...
/* The industry valuation resource, folded into one line per industry.  Two fields in the
 * same resource describe membership - the declared $S_ZQDM list and the rows themselves -
 * so both counts are reported with a flag saying whether they agree. */
static int command_industry(const cli_options *options, tdx_error *err) {
    static tdx_jsn_document document;
    static tdx_buf raw;
    static tdx_buf utf8;
    static tdx_buf line;
...
/* Index valuation.  Without --security this is the eleven-row current table; with one it
 * also fetches the three detail resources the row's own detail id names, merges the PE and
 * PB histories by date, and reports how the two halves joined. */
static int command_valuation(const cli_options *options, tdx_error *err) {
    static tdx_jsn_document master;
    static tdx_buf raw;
    static tdx_buf utf8;
    static tdx_buf line;
            ...
            for (index = 0; index < 3; ++index) {
                 ...
                 * here builds it. */
                {
                    ...
                    if (tdx_jsn_remote_path(remote, "bi", prefixed, sizeof(prefixed), err) !=
                        TDX_OK)
                        goto done;
                    snprintf(remote, sizeof(remote), "%s", prefixed);
                }
                tdx_buf_clear(&raw);
                tdx_buf_clear(&utf8);
                memset(&info, 0, sizeof(info));
                if (tdx_download_resource(&options->pool, options->timeout_ms, remote, 1, &raw,
...
int main(int argc, char **argv) {
    ...
    if (strcmp(argv[1], "valuation") == 0) {
        if (command_valuation(&options, &error) != TDX_OK) {
            fprintf(stderr, "error: %s\n", error.message);
            return 1;
        }
        return 0;
    ...
    if (strcmp(argv[1], "industry") == 0) {
        if (command_industry(&options, &error) != TDX_OK) {
            fprintf(stderr, "error: %s\n", error.message);
            return 1;
        }
        return 0;
