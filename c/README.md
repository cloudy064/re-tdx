...
1. **不改**共享除数表：它是协议策略、被所有命令使用，未经充分测量改成 `13 → 100` 会波及
   `130`/`131`/`133`… 全部代码，风险大于收益。
2. **标注**：定价视图新增 `price_plausible`，债券价 > 10 倍面值时置 false。**数字照原样输出**
   （隐藏数字只会让缺陷消失而不是被看见），只是告诉读者别信它。实测 312 行中**恰好 2 行**被标注。
3. **留余地**：阈值取 10 倍面值而非 2 倍——可转债真的涨到过 3000 元。实测 `SH113615` 报价
   **738 元**（其日线 671.657 元，同量级）**没有**被标注，说明这个阈值抓到了单位错误、
   又没有冤枉真实的高价债。
