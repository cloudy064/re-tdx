...
| 板块层级展开（`data/blocks_loader.cpp` + `blocks.hpp`） | 板块家族/父子层级/成员并集 | **在范围内，待做**（见下） |
...
**板块层级展开在范围内**（第 15 条，尚未做 ⇒ **目标未完成**）；
**扩展市场分钟线范围外**（第 13 条，实测公开会话不应答）。
所以这里**没有一条属于"其实能做只是没做"**，除第 15 条之外。
