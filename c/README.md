...
**本次审计（第 20 轮末）**：`market/` 下**在范围内的模块清单已清空**。
下面剩下的每一项都经过一次核对，分成三类——**外部阻塞**（1、2、5、8、9）、
**有意不做**（11：会破坏"部署就是单个 exe + zlib"）、
**已判定为未知且参考树也未知**（3、4、6、7）。
其中 3/4/6/7 是拿 `native/` 参考树当消费者证据逐条查过的，结论写在各自条目里：
**没有一条是"其实能做只是没做"**。
...
13. **扩展市场（期货/期权）的分钟线**：`MinuteBar` 带 `open_interest`/`auxiliary_price`，
    ...
    （参考的 `pack_lc1` 因此直接拒绝），属于扩展市场那一层，**范围归属未判定**。
14. **板块层级展开**：`hyzt` 之上还有一层父子板块/成员并集/层级树，依赖
    `tdx/blocks.hpp` 与一路 cloud 数据源；**其范围归属尚未判定**。
    （注：这两条此前都编成了 13，本次审计一并改正。）
