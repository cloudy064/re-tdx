...
c/
  ...
  tests/
    ...
    test_professional_finance.c  professional_finance_fixtures.h
                      (deterministic Python ZIPs checked by zipfile, read by this code)
  tools/
...
本模块交付**算术层**；**行映射与报价联接**由 `pricing` 命令完成（定价资源与 `0x054C` 快照
按市场+代码联接，再算全价 / 转股价值 / 溢价 / 纯债价值）。实测输出含
`full_price` / `conversion_value` / `pure_bond_value` 等字段。
...
| 参考模块 | 内容 | 判定 |
|---|---|---|
| `market/daily.cpp` | 本地 `.day` 日线（32 字节记录） | **已交付** → 命令 `daily` |
| `market/minute.cpp`、`local_kline.cpp` | 本地分钟线（`.lc1`） | **已交付** → 命令 `minute` |
| `minute_download*.cpp`（A 股半边） | 分钟线下载与 `.lc1` 落盘 | **已交付** → `kline --lc1-output` |
| `market/hyzt.cpp` | 行业估值 JSN | **已交付** → 命令 `industry` |
| `market/panorama.cpp` | 全景 JSN 查询 | **已交付** → 命令 `panorama` |
| `market/ranking.cpp` | 分类行情/排名（走 7709 传输） | **已交付** → 命令 `ranking` |
| `market/seal_order.cpp` | 涨跌停规则 + 封单 | **已交付** → 命令 `limit` 与 `seal` |
| `market/valuation.cpp` | 估值 JSN 查询 | **已交付** → 命令 `valuation` |
...
若它仍写着"待做"，那说明表格本身过期了，而不是工作没做——本表在第 22 轮统一核对过一次。

### 第 21 轮补判的两条（此前列为"范围归属未判定"）

**① 板块层级展开 → 在范围内。** 原先写的是"依赖 `tdx/blocks.hpp` 与一路 cloud 数据源"，
**那句话是错的**：`hyzt.cpp:308` 用的是 `load_blocks(root, {"industry"})`，传的是**本地 root**；
...
| 命令 | 结果 |
|---|---|
| `0x0547`（对照组，本项目已在用） | **应答 117 字节** ✓ 会话正常 |
| `0x23F0` | 超时无应答（`WSA 10060`） |
| `0x23F5` | 超时无应答 |
| `0x23FA` | 超时无应答 |

`0x23F0` 是**按参考原样发空 body**（参考就是 `connection.call(type)` 不带数据），仍然没有应答
⇒ 不是请求体写错，而是**这条命令不向公开会话提供**。本机也不具备它的数据
（`vipdoc/ds/lday`、`vipdoc/ds/minline` **都是空的**）。
⇒ 与 `level2_*.cpp` 同一类：公开会话之外的条件。**范围外。**

> 探针第一版报"四条全部失败"，**包括对照组**——两个错误：对照组用了非法的空 `0x0547` 请求；
> 而那条失败之后连接已不可用，后三条**根本没发出去**。改成每条命令各开一条连接、
> 对照组用真实盘口请求之后，结论才有意义。**一个失败的对照组会把整组测量变成噪声，
> 而它看起来和"结论"一模一样。**
...
**与本机三份本地缓存文件**。本轮只做行业估值这一层；
第 21 轮判定它在范围内，第 22 轮**已交付**（命令 `blocks`，见"板块层级"一节）。
（原文此处写的是"依赖一路 cloud 数据源、范围归属尚未判定"——**两处都不对**，
范围判定一节里有实测依据。）
...
**已交付**（见"封单"一节，命令 `seal`）：由五档盘口与竞价不平衡量算封单金额与占比，
三条路径（竞价不平衡 / 竞价二档 / 连续）各自有测试，并对真实盘口验证过。
...
| `test_professional_finance` | ZIP 由 **独立 Python 生成器确定性构造，经 zipfile/zlib 校验，再由本代码读**（stored 与 deflate 两条路径）、**破坏 CRC 必须被拒绝**、缺 EOCD 被拒绝、定长表头/索引/数据起点逐字段断言、**字段取值按 `index` 与 `index×2` 设计**使偏移错位必然暴露、越界字段为缺失而非 0、成员结构错误的四种拒绝、只命名两个字段、**渲染行必须能被项目自己的 JSON 解析器解析**（这条抓出了两个真 bug） |
...
**第 20 轮末审计**：`market/` 下**在范围内的模块清单已清空**；余下各项分三类——
**外部阻塞**（1、2、5、8、9）、**有意不做**（11：会破坏"部署就是单个 exe + zlib"）、
**已判定为未知且参考树也未知**（3、4、6、7，逐条拿 `native/` 当消费者证据查过）。

**第 21 轮补判**：此前列为"范围归属未判定"的两条已判完——
**板块层级在范围内**（第 15 条），**扩展市场分钟线范围外**（第 13 条，实测公开会话不应答）。

**第 22 轮**：板块层级**已交付**（命令 `blocks`，见"板块层级"一节）。
于是这里**没有一条属于"其实能做只是没做"**——余下每一条都是外部阻塞、有意不做，
或已判定为未知且参考树也未知。
...
3. **指定日期分时 `0x0FB4`**：请求格式还没复原。范围已收窄到"11 字节体被识别、那 4 个
   ...
   **参考树里根本没有 `0x0FB4`**（审过一遍），所以"从参考读出它的调用点"这条路不存在，
   只能从 TdxW 二进制或被动观察取证据。
4. **两条 L1 命令各留一大段未建模字节**：`0x0547` 的 46 字节、`0x054C` 的 61–62 字节尾部。
   本实现只报数量、不编语义。
   **参考树里同样是未知**：它把那个字段叫 `unknown_after_outer`，并把它当作竞价不平衡量用
   ——与本实现的处理一致。顺带排掉一个同数陷阱：参考里另外那处"exactly 46 bytes"是
   **L2 SDK 的伴生快照输入**，与 `0x0547` 的尾部毫无关系。
5. **L2 秒级逐笔成交 / 逐笔委托**：交易所口径的那个，走内置 `1364`/`1374` 或 SDK
   `4655`/`1801`/`1802`，需要授权业务事件。本仓库只做到结构确认与被动探针，`c/` 侧没做
   ——公开会话拿不到，做了也无法验证。
6. **`0x0010` 里三个未标定的股本类别槽位**：
   **参考树给了名字**（`national` / `promoter_legal_person` / `legal_person`，见 `corporate.cpp`），
   **但实测的数值与这些名字矛盾**——在工行、茅台身上给出不可能是股本的数。
   所以本实现**继续不命名**：给一个数据已经反驳的名字，比留着三个数字更糟。
   这条现在记录的是"参考说它叫什么、数据说它不是那个"，而不是"没人知道"。
7. **`1i..1m` 阶段辅助块还没有语义**——**这一条其实早有答案，本次审计确认了它**：
   大写邻居 `1G/1H`（委买均价/总买量）与 `1I/1J`（委卖均价/总卖量）**已经收录并命名**
   （见"本地 `.img`"一节，证据是 TdxW 界面标签表 `0xCA1330..0xCA1348`，且有
   `1G ≤ 最新价 ≤ 1I` 的实测排序关系）。清单里这条指的是**小写的**那一族，
   与它们是**不同的块**，仍然没有语义。
...
12. **`market/` 下在范围内的模块**：`daily`、`minute`、`hyzt`、`valuation`、`ranking`、
    `seal_order`、`panorama`、`blocks`、以及 `minute_download*` 的
    **A 股分钟线下载与 `.lc1` 落盘**均已交付。**这一项已清空。**
13. **扩展市场（期货/期权）的分钟线**：**已判为范围外**（第 21 轮，有实测依据）。
    参考用 `0x23F0`/`0x23F5`/`0x23FA` 这套 "tdx-7727" 传输，公开会话**不应答**
    （对照组 `0x0547` 同一轮应答 117 字节）；本机 `vipdoc/ds/` 下也没有它的分钟数据。
    详见上文范围判定一节。
