...
The finance ZIP generator independently constructs its table and explicit ZIP records
through Python's `struct`. All metadata is fixed, including timestamp, platform,
versions and permissions. A small deterministic fixed-Huffman/LZ77 encoder produces
the method-8 member with actual compression; generated bytes do not depend on zlib's
compression heuristics or `zipfile` writer defaults. Python's `zipfile` and zlib then
independently verify the metadata, decoded contents, CRC rejection and missing-end-record
rejection. Stored and compressed ZIP reader paths remain separate fixtures.
